...
<body>
<div id="content">
    <div id="navigation">当前位置：<a href="#">首页</a><span>>></span><a href="#">后台管理</a><span>>></span><a href="#">站内公告</a>
    </div>
    <form action="<%=request.getContextPath() %>/stationAnnouce/searchStationAnnouces.do" method="post"
          name="annouceForm" onsubmit="return isSubmit()">
        <table class="myshop">
            <tr>
                <td style="text-align:center;font-size:15px;" colspan="5">公告时间：
                    ...
                    <a href="<%=request.getContextPath() %>/stationAnnouce/beforeAddStationAnnouce.do"
                       target="mainContent">增加公告</a>
                </td>
            </tr>
                ...
                <c:forEach items="${stationAnnouce}" var="stationAnnouce" varStatus="status">
                    <tr>
                        ...
                        <td style="text-align:center;"><a
                                href="<%=request.getContextPath() %>/stationAnnouce/selectStationAnnouce.do?id=${stationAnnouce.id} ">查看</a>
                            <c:if test="${stationAnnouce.state==1 }">
                                <a href="<%=request.getContextPath() %>/stationAnnouce/sendStationAnnouces.do?id=${stationAnnouce.id}"
                                   onClick="return confirm('是否发送?')">发送</a>
                            </c:if>
                            <a href="<%=request.getContextPath() %>/stationAnnouce/deleteStationAnnouces.do?id=${stationAnnouce.id }"
