...
<body>
<div id="content">
    <div id="navigation">当前位置：<a href="#">首页</a><span>>></span><a href="#">后台管理</a><span>>></span><a href="#">查看已下订单</a>
    </div>
    <form action="<%=request.getContextPath() %>/order/searchOwnOrders.do" method="post" name="orderForm">
        <table border="0" cellspacing="0" class="myshop">
            <tr>
                <td style="text-align:center; font-size:15px; height:30px;" colspan="9">订单号：
                    <input type="text" name="orderNum"/>&nbsp;&nbsp;
                    订货时间：
                    <input type="text" name="startTime" id="startTime" style="width: 80px;" readonly="readonly"
                           onClick="WdatePicker()"/>
                    ——
                    <input type="text" name="endTime" style="width: 80px;" readonly="readonly" onClick="WdatePicker()"
                           onfocus="checkStartDate()"/>
                    &nbsp;&nbsp;状态：
                    <select name="state">
            ...
            <tr>
                <td style="text-align:center; font-size:15px; height:30px;" colspan="9"><input type="submit" value="搜索"
                                                                                               onfocus="checkDate()"/>
            ...
            <c:if test="${page.num!=0}">
                <tr>
                    ...
                    <td style="text-align:center;font-size:15px;">商家店铺</td>
                    <td style="text-align:center;font-size:15px;">商家昵称</td>
                    <td style="text-align:center;font-size:15px;">状态</td>
                ...
                <c:forEach items="${goodsOrder}" var="goodsOrder" varStatus="status">
                    <tr>
                        ...
                        <td style="text-align:center;font-size:15px;">${goodsOrder.good.shop.name }</td>
                        <td style="text-align:center;font-size:15px;">${goodsOrder.good.shop.user.callName }</td>
                        ...
                        <td style="text-align:center;font-size:15px;"><a
                                href="<%=request.getContextPath()%>/order/selectOwnOrders.do?id=${goodsOrder.id}">查看</a>
                            <c:if test="${goodsOrder.state!=3 }">
                                <a href="<%=request.getContextPath()%>/order/takeDelivery.do?id=${goodsOrder.id }"
                                   onClick="return confirm('是否已收到购买物品?')">已收到</a>
                            </c:if>
                        </td>
                    </tr>
                </c:forEach>
                <tr>
                    <td colspan="9" align="center">
                        ...
                        <c:if test="${page.hasPrePage}">
                            <a href="<%=request.getContextPath()%>/order/searchOwnOrders.do?page=1">首页</a> |
                            <a href="<%=request.getContextPath()%>/order/searchOwnOrders.do?page=${page.currentPage-1 }">上一页</a> |
                        </c:if>
                        <c:if test="${!page.hasNextPage}">下一页 | 尾页 | </c:if>
                        <c:if test="${page.hasNextPage}">
                            <a href="<%=request.getContextPath()%>/order/searchOwnOrders.do?page=${page.currentPage+1 }">下一页</a> |
                            <a href="<%=request.getContextPath()%>/order/searchOwnOrders.do?page=${page.num }">尾页</a> |
                        </c:if>
            ...
            <tr>
                <td colspan="9" align="center" height="50" valign="middle">
