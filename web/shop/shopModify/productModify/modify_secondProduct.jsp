...
<head>
    ...
    <script type="text/javascript">
        ...
        function ajaxFileUpload() {
            ...
            $.ajaxFileUpload(
                {
                    url: '<%=path %>/shop/modifyHotGoods.do?x1=' + x1 + '&y1=' + y1 + '&w=' + w + '&h=' + h + '&userName=' + userName + '&shopId=' + shopId + '&goodsType=' + goodsType + '&goodsId=' + goodsId,            //需要链接到服务器地址
                    secureuri: false,
                    type: "post",
...
<div id="content" style="width:950px;margin-left: 25px; margin-right: 27px;">
    <form action="<%=path %>/system/modifyUserGoods.do" method="post" id="addOtherForm" onSubmit="return  isModify();">
        ...
        <input name="jspType" value="secondModify" type="hidden">
        <table class="myshop" cellspacing="0" style="width: 100%;">
            <tr>
                <th colspan="5">店铺<span>>></span>店铺装修<span>>></span>修改最新商品</th>

            </tr>
            <tr>
                <td style="">商品名称：</td>
                <td><input type="text" id="name" name="name" onBlur="checkName()" style="width: 130px;"
                           value="${goods.name}"/><span style="color: red;">*</span></td>
            ...
            <tr>
                ...
                <td>
                    商品价格：<input type="text" id="price" name="price" onBlur="checkPrice()" value="${goods.price}"/><span
                        style="color: red;">*</span><span>单位为元</span>

                </td>
                <td>
                    商品尺寸：<input type="text" id="size" name="size" onBlur="checkSize()"
                                value="${goods.size}"/><br/><span>尺寸单位：<input type="text" id="SizeUnit" name="sizeUnit"
                                                                              onBlur="checkSizeUnit()"
                                                                              value="${goods.sizeUnit}"/></span>&nbsp;
                </td>
                <td>商品数量：
                    <input type="text" id="amount" name="amount" onBlur="checkAmount()" value="${goods.amount}"/>
                    <span style="color: red;">*</span><br/><span>数量单位：
                      <input onBlur="checkAmountUnit()" type="text" id="amountUnit" name="amountUnit"
                             value="${goods.amountUnit}"/></span>&nbsp;<span style="color: red;">*</span></td>
                <td> 商品产地：<input type="text" id="origin" name="origin" onBlur="checkOrigin()" value="${goods.origin}"/>&nbsp;</br>
                </td>

            </tr>
            <tr>
                <td>商品毛重：<input type="text" id="weight" name="weight" onBlur="checkWeight()"
                                value="${goods.weight}"/><br/><span>毛重单位：<input type="text" id="grossUnit"
                                                                                name="grossUnit"
                                                                                onBlur="checkGrossUnit()"
                                                                                value="${goods.grossUnit}"/></td>
                <td>商品品牌：<input type="text" id="brand" name="brand" onBlur="checkBrand()" value="${goods.brand}"/></td>
                <td>商品颜色：&nbsp;&nbsp;<input type="text" id="color" name="color" onBlur="checkColor()"
                                            value="${goods.color}"/></td>
                <td colspan="2">生产厂家：<input type="text" id="productAdd" name="productAdd" onBlur="checkProductAdd()"
                                            value="${goods.productAdd}"/></td>
            </tr>
            ...
            <tr>
                ...
                <td colspan="4" style="height: 270px;padding-left: 20px;">
                    ...
                    <div id="add">
                        ...
                        <div>(<span style="color: #FF8080; font-size: 12px;">注:建议宽为369px，高为246px(3：2),大小不超过100KB,请上传gif,jpg格式.</span>)
                        </div>
                        ...
                        <input type="button" value="上传图片" onClick="ajaxFileUpload(add)"
                               style="width: 60px; margin-left: 50px;"/>&nbsp;<span style="color:red; font-size: 12px;"
                                                                                    id="updownInfo">请先上传图片！</span><img
            ...
            <tr>
                <td>商品描述</td>
                <td colspan="4"><textarea rows="18" style="width:850px;height: 150px;" id="content1"
                                          name="description"></textarea><span id="word_count"></span>
            ...
            <tr style="text-align:center;">
                <td colspan="5"><input type="submit" value="提交" style="vertical-align: middle;"/>&nbsp;<a
                        href="<%=path %>/shop/selectRollingGoods.do?shopId=${user.id }&typeJsp=selecHotGoods"
                        style="color: blue;margin-right: 120px;float: right;">返回查看页面</a></td>
            </tr>
        </table>
    </form>
</div>
