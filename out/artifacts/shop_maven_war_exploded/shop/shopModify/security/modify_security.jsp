...
$(function(){
		...
		if('${!empty updateSecureSuccess && !empty shop.secureShop}'=='true'){

			$("#modifySecureSuccess").show();
		}else{
		//否则不显示“装饰商家介绍信息成功,请先关闭窗口当前窗口，然刷新页面”
			$("#modifySecureSuccess").hide();
...
<style type="text/css">
	...
	*{margin: 0px; padding: 0px; }
	...
	#modifySecureSuccess{height:40px; line-height:40px; font-family:宋体; font-size:14px; color:#FF7D7D;}	
	-->
</style>
</head>
...
<center>
<div>
	<form action="<%=request.getContextPath()%>/modifySecure.do?userId=${userId}" id="myForm" method="post">
		<fieldset>
		<legend>购物保障</legend>
		<div id="if_add">您还没有添加商家介绍信息！<a href="javascript:void(0)" id="add" >马上添加</a></div>
		<div id="secureContent">
			<textarea name="content" cols="21" rows="10" id="content1" style="width:100%">${shop.secureShop}</textarea>
