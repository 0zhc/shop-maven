...
<body>
<div  id="content">
<div id="navigation">当前位置：<a href="#">首页</a><span>>></span><a href="#">后台管理</a><span>>></span><a href="#">查看已下订单</a></div>
...
<select name="state">
  ...
  <option value="3">已交易</option>
		...
		<c:forEach items="${goodsOrder}" var="goodsOrder" varStatus="status">
            ...
            <td style="text-align:center;font-size:15px;"><a href="<%=request.getContextPath()%>/order/selectOwnOrders.do?id=${goodsOrder.id}">查看</a>
            <c:if test="${goodsOrder.state!=3 }">
            <a href="<%=request.getContextPath()%>/order/takeDelivery.do?id=${goodsOrder.id }" onClick="return confirm('是否已收到购买物品?')">已收到</a>
            </c:if>
            </td>
          </tr>
          </c:forEach>
