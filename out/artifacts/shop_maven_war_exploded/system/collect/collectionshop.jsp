...
<body>
<div  id="content">
	<div id="navigation">当前位置：<a href="#">首页</a><span>>></span><a href="#">后台管理</a><span>>></span><a href="#">店铺收藏</a></div>
	<form id="userform">
		<table  class="myshop">
			<tr >
			...
			<c:forEach items="${collectionShopList}" var="c">
				<tr >
					...
					<td style="text-align:center;"><a href="<%=request.getContextPath()%>/shop.do?id=${c.shops.id }" target="_top">${c.shops.name}</a></td>
						...
						<c:when test="${!empty collectionShopList}">
							...
							<c:forEach begin="${pager.currentPageStart}" end="${pager.currentPageEnd}" var="count">
								<c:if test="${count<=pager.totalPages}">
									<a href="javascript:shopPage('${count}','${userId}','${shopName}','${datetime1}','${datetime2}');" class="ACountClass">${count}</a>
								</c:if>
							</c:forEach>
