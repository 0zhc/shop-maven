...
<div id="navigation">当前位置：<a href="#">首页</a><span>>></span><a href="#">后台管理</a><span>>></span><a href="#">他人投诉结果</a></div>
      <form action="#">
		...
		<c:forEach items="${complaintAlerts}" var="complaintAlerts" varStatus="status">
            ...
            <td style="text-align:center;font-size:15px;"><a href="<%=request.getContextPath() %>/complaint/selectAdminComplaintAlert.do?id=${complaintAlerts.id}">查看</a><a href="<%=request.getContextPath() %>/complaint/deleteAdminComplaintAlert.do?id=${complaintAlerts.id }" onClick="return confirm('是否删除?')">删除</a></td>
          </tr>
          </c:forEach>
          <tr>
