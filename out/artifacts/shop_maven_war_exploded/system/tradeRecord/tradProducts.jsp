...
<div id="navigation">当前位置：<a href="#">首页</a><span>>></span><a href="#">后台管理</a><span>>></span><a href="#">商品已买</a></div>
     <form action="<%=request.getContextPath()%>/tradeRecord/searchTradProducts.do" method="post" name="searchForm" onsubmit="isSubmit()">
        <table  class="myshop">
...
交易时间：
<input type="text" name="startTradeTime" id="startTradeTime" readonly="readonly" onClick="WdatePicker()"/>
——
<input type="text" name="endTradeTime" readonly="readonly" onClick="WdatePicker()" onfocus="checkStartTradeDate()" />
				<input type="hidden" name="page" value="1">
			</td>
          </tr>
          <tr >
           <td style="text-align:center; font-size:15px; height:30px;" colspan="10"><input type="submit" value="搜索" onfocus="checkDate()" />
...
<%--				<td style="text-align:center;font-size:15px;"><input type="checkbox"  onclick= "change(document.getElementsByName('check'), this.checked)"/>全选</td>--%>
		...
		<c:forEach items="${tradeRecord}" var="tradeRecord" varStatus="status">
            ...
            <td style="text-align:center;font-size:20px;" ><a href="<%=request.getContextPath()%>/tradeRecord/getTradProducts.do?id=${tradeRecord.id}">投诉商家</a></td>		
            </c:if>
            <c:if test="${tradeRecord.preRightsId==0}">
            <td style="text-align:center;font-size:20px;" ><a href="<%=request.getContextPath()%>/complaint/selectComplaint.do?tradeId=${tradeRecord.id}">查看</a></td>		
            </c:if>
            <td style="text-align:center;"><a href="<%=request.getContextPath()%>/tradeRecord/deleteTradProducts.do?id=${tradeRecord.id}&delType=1" onClick="return confirm('是否删除?')">删除</a></td>
          </tr>
        </c:forEach>
		      <tr>
        ...
        <tr>
         ...
         <tr><td colspan="10" align="center" height="50" valign="middle">
