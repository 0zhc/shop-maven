...
<style type="text/css">
	.ACount{color:blue;}
	a{text-decoration:none; color:#000000; margin-left:3px;}
      ...
      <form id="userform">
        <table  class="myshop">
          <tr style="background-image:url(../images/sysytem/tableTip.jpg); background-repeat:no-repeat; line-height:21px;">
            <td colspan="9" ><a href="#">首页</a><span>>></span><a href="#">后台管理</a><span>>></span><a href="#">店铺管理</a></td>
          </tr>
           ...
           <tr >      
				 <td style=" text-align:center; font-size:15px; height:30px;" colspan="8">
					...
					店主：<input type="text"  name="shopUser" maxlength="35" value="${shopUser}" style="width: 90px;">&nbsp;&nbsp;
                 	开店时间：<input type="text" name="datetime1" value="${datetime1}" readonly="readonly" style="width: 95px;" onClick="WdatePicker()"/>&nbsp;——&nbsp;<input type="text" name="datetime2" value="${datetime2}" readonly="readonly" style="width:95px;" onClick="WdatePicker()"/>
		...
		<c:forEach items="${shopList}" var="s">
           <tr height="30px">				  
              ...
              <td style="text-align:center;font-size:15px;" ><a href="<%=request.getContextPath()%>/shop.do?id=${s.id}" target="_top">${s.name}</a></td>
       ...
       <c:choose>
        ...
        <c:forEach begin="${pager.currentPageStart}" end="${pager.currentPageEnd}" var="count">
          <c:if test="${count<=pager.totalPages}">
           <a href="<%=request.getContextPath()%>/shopManagerSelectAllShop.do?currentpage=${count}&shopName=${shopName}&shopUser=${shopUser}&datetime1=${datetime1}&datetime2=${datetime2}" class="ACountClass">${count}</a>
          </c:if>
        </c:forEach>
